...
echo $OUTPUT->heading(get_string($saas->config->course_mapping, 'report_saas_export'), '4');

echo html_writer::start_tag('TABLE', array('class'=>'table table-hover'));
echo html_writer::tag('TH', html_writer::tag('H4', 'Ofertas de Cursos/Ofertas de disciplinas'));
echo html_writer::tag('TH', html_writer::tag('H4', 'Cursos Moodle'));
...
if ($saas->config->course_mapping == 'one_to_many') {
    ...
    foreach ($ofertas_de_curso as $oferta_de_curso) {
        ...
        echo html_writer::tag('button', 'Adicionar', array('type'=>'button', 'class'=>'btn btn-default btn-xs saas_map_bt',
                                                           'style'=>'margin-top:5px;', 'id'=>$oferta_de_curso->uid));
        echo html_writer::end_tag('td');

        echo html_writer::start_tag('td');
        echo html_writer::tag('button', 'Adicionar', array('type'=>'button', 'class'=>'btn btn-default btn-xs moodle_map_bt', 'style'=>'margin-top:5px;'));
        echo html_writer::end_tag('td');
...
//Mapeamento de uma oferta do SAAS para 1 ou mais cursos Moodle.
    ...
    foreach ($ofertas_de_curso as $oferta_de_curso) {
        ...
        echo html_writer::tag('td', $nome_formatado, array('id' => $oferta_de_disciplina->id, 'style'=>'text-indent: 30px;', 'bgcolor'=>'#F0F0F0'));
        ...
        if (array_key_exists($oferta_de_disciplina->id, $ofertas_mapeadas_com_cursos)) {
            ...
            if ($saas->config->course_mapping == 'many_to_one') {
                ...
                echo html_writer::tag('button', 'Adicionar', array('type'=>'button', 'id'=>$oferta_de_disciplina->id,
                        'class'=>'btn btn-default btn-xs moodle_map_bt', 'style'=>'margin-top:5px;', 'oferta'=>$nome_formatado));
                echo html_writer::end_tag('div');
            }
                ...
                echo html_writer::start_tag('div');
                echo html_writer::tag('button', 'Adicionar', array('type'=>'button', 'id'=>$oferta_de_disciplina->id,
                            'class'=>'btn btn-default btn-xs moodle_map_bt', 'oferta'=>$nome_formatado));
                echo html_writer::end_tag('div');
            }
            echo html_writer::end_tag('div');
            echo html_writer::end_tag('td');
