...
<?php
$ofertas_de_curso = $saas->get_ofertas_curso();
...
if ($saas->get_config('course_mapping') == 'one_to_many') {
    ...
    foreach ($ofertas_de_curso as $oferta_de_curso) {
        ...
        $sql_cursos_mapeados = "SELECT DISTINCT map.courseid
                                  ...
                                  JOIN {saas_ofertas_disciplinas} od
                                    ON (map.oferta_disciplina_id = od.id AND
                                        od.enable = 1 AND
                                        od.oferta_curso_uid = :oc_uid)";
        ...
        foreach ($cursos_mapeados as $cm) {
            $sql_od = "SELECT od.*,  map.*, dis.nome
                         ...
                         JOIN {saas_map_course} map ON (od.id = map.oferta_disciplina_id AND map.courseid = :map_courseid)
                        WHERE od.enable = 1";
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <?php
            ...
            saas_build_tree_categories($repeat_allowed);
          ?>
        </div>
